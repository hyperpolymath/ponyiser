...
ponyiser eliminates data races by wrapping concurrent code in Pony actor
and behaviour wrappers with reference capability annotations. Pony’s six
reference capabilities (iso, val, ref, box, trn, tag) guarantee
data-race freedom at compile time without locks or runtime checks.
ponyiser reads concurrency patterns from a `+ponyiser.toml+` manifest
and generates Pony actor wrappers with the appropriate capability
annotations, targeting any codebase that needs provably safe
concurrency.
